...
- [x] Запустить `pytest tests/ -v` — все тесты зелёные
- [x] Запустить `ruff check src/ tests/` — без ошибок
- [x] Запустить smoke-test конфига и убедиться в разумном числе источников (ожидаем 45–55 enabled)
- [x] Проверить что в `config.yaml` есть блок документации к полю `priority` (по аналогии с `enabled`)
- [x] В `README.md`: обновить количество источников, если упомянуто явно
- [x] Mark completed

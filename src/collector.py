...
async def collect(
    ...
    if source_stats is not None:
        for source, raw_articles in zip(config.enabled_sources, results, strict=False):
    ...
    successful_sources = [
        s for s, r in zip(config.enabled_sources, results, strict=False) if r is not None
    ]
    ...
    for source, raw_articles in sorted(
        zip(config.enabled_sources, results, strict=False),
